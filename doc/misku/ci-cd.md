...
- Pull requests targeting `main` restore dependencies, compile the x64 Release package, run settings and command-line tests, validate the runtime payload, and upload a 30-day artifact.
- Every push to `main` performs the same checks and publishes an incremental prerelease named `misku-v0.2.<run number>`.
...
Run the same regression tests after building:
