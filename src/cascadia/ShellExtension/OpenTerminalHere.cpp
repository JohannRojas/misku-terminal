...
try
{
...

    {
        ...
        std::wstring cmdline;
        cmdline.reserve(256); // The misku.exe path is ~110 characters long
        cmdline.push_back(L'"');
...
{
    std::filesystem::path modulePath{ wil::GetModuleFileNameW<std::wstring>(wil::GetModuleInstanceHandle()) };
    // misku.exe,-101 will be the first icon group in WT
    ...
    modulePath.replace_filename(L"misku.exe,-101");
